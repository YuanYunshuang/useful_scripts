...
def show_bbxs_rectangles(img, bbxs):
    bbxs = bbxs.astype(np.int32)
    for i in range(len(bbxs)):
        cv2.rectangle(img, (int(bbxs[i, 1]), int(bbxs[i, 0])), (int(bbxs[i, 3]), int(bbxs[i, 2])), (255, 0, 0), 1)
    cv2.imshow('tmp', img)
    cv2.waitKey()
    ...
    for _ in range(40):
        ...
        with open(os.path.join(save_path, 'labels', label_str + '.txt'), 'w') as fh:
            for i in range(bbxs.shape[0]):
                ...
                fh.write(l + ' ' + str(x_min) + ' ' + str(y_min) + ' ' + str(x_max) + ' ' + str(y_max) + '\n')
...
class RandConfigTxt:
    ...
    def generate_random_config(self):
        ...
        self.thickness = random.randint(5, 8)
