...
def show_bbxs_polygons(img, bbxs):
...
def show_bbxs_rectangles(img, bbxs):
    bbxs = bbxs.astype(np.int32)
    for i in range(len(bbxs)):
        cv2.rectangle(img, (int(bbxs[i, 0]), int(bbxs[i, 1])), (int(bbxs[i, 2]), int(bbxs[i, 3])), (255, 0, 0), 1)
    cv2.imshow('tmp', img)
    cv2.waitKey()
...
def noisy(noise_typ,image):
    ...
    if noise_typ == "gauss":
      ...
      row,col,ch = image.shape
      gauss = np.random.randn(int(row*col/2),ch)
      for g in range(gauss.shape[0]):
          x = np.random.randint(0,row)
          y = np.random.randint(0,col)
          image[x,y,:] = image[x,y,:] + image[x,y,:] * gauss[g,:]
      return np.clip(image,0,255)

def generate_label1(save_path):
    txt_cfg = RandConfigTxt()
    char_list = list(string.digits) + list(string.ascii_uppercase) + list('-')
    label_map = dict(zip(char_list,range(len(char_list))))
    for _ in range(400):
        indices = np.random.randint(0,len(char_list), np.random.randint(1,10))
        label_str = ''.join(np.array(char_list)[indices])
        if not ('B' in label_str or 'I' in label_str or 'Y' in label_str \
                or 'V' in label_str or '-' in label_str or 'D' in label_str):
            if np.random.rand() > 0.2:
                continue
        txt_cfg.generate_random_config()
        img, bbxs = generate_label_raw(label_str, txt_cfg)
        #show_bbxs_polygons(img.copy(), bbxs)
        cv2.imwrite(os.path.join(save_path, 'images', label_str + '.png'), img)
        bboxes = []
        with open(os.path.join(save_path, 'labels', label_str + '.txt'), 'w') as fh:
            for i in range(bbxs.shape[0]):
                x_min = int(min(bbxs[i,0::2]))
                x_max = int(max(bbxs[i,0::2]))
                y_min = int(min(bbxs[i,1::2]))
                y_max = int(max(bbxs[i,1::2]))
                l = label_map[label_str[i]]
                bboxes.append([x_min, y_min, x_max, y_max])
                fh.write(str(l) + ' ' + str(x_min) + ' ' + str(y_min) + ' ' + str(x_max) + ' ' + str(y_max) + '\n')
        #show_bbxs_rectangles(img.copy(), np.array(bboxes))
...
def generate_label_raw(label_str, txt_cfg):
    ...
    for char in label_str:
        r1, _ = cv2.getTextSize(char, txt_cfg.font, txt_cfg.scale, txt_cfg.thickness)
        widths.append(r1[0])
    ...
    s = (sum(widths) - ret[0]) / (len(label_str) - 1) if len(label_str)>1 else 0
    ...
    TYPES = ['gauss', 's&p']
    n_types_idxs = np.random.randint(0,2,np.random.randint(1,2))
    img = noisy_image(img, np.take(TYPES,n_types_idxs))
    k = np.random.randint(2,5) * 2 + 1
...
class Transformation:
    ...
    def rotate(self, img, bbxs):
        ...
        img = cv2.copyMakeBorder(img, dy, dy, dx, dx, cv2.BORDER_CONSTANT, value=self.margin_color)
        M = cv2.getRotationMatrix2D((cols / 2, rows / 2), angle, 1)
        rows, cols = img.shape[:2]
        bbxs = bbxs + np.tile([dx-cols//2, dy-rows//2], 4)
        dst = cv2.warpAffine(img, M, (cols, rows), borderValue=self.margin_color)
    ...
    def project(self, img, bbxs):
        ...
        img_output = cv2.warpPerspective(img, P, (cols, rows), borderValue=self.margin_color)
        for i in range(0,8,2):
...
generate_label1('/home/robotics/OCR_synthetic_data/sythetic_data_A')
